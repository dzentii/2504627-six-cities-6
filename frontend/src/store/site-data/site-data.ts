    ...
    builder
      ...
      .addCase(postFavorite.fulfilled, (state, action) => {
        ...
        if (updatedOffer.isFavorite) {
          const favoriteExists = state.favoriteOffers.some((offer) => offer.id === updatedOffer.id);

          if (!favoriteExists) {
            const offerFromState = state.offers.find((offer) => offer.id === updatedOffer.id);
            state.favoriteOffers = state.favoriteOffers.concat(offerFromState ?? updatedOffer);
          }
        } else {
